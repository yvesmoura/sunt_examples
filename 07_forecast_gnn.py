"""
07 – Spatio-Temporal GNN forecast (T-GCN) for the bus network.
...
# Training curves
axes[0].plot(history["train"], label="Train MSE")
axes[0].plot(history["val"],   label="Val MSE")
axes[0].set_xlabel("Epoch"); axes[0].set_title("T-GCN – training curves")
axes[0].legend()

# Predicted vs actual for one node (first few test samples)
node_idx  = 0
n_samples = min(200, len(pred_flat))
axes[1].plot(true_flat[:n_samples, node_idx], label="Actual", color="black", lw=1)
axes[1].plot(pred_flat[:n_samples, node_idx], label="T-GCN", color="teal", lw=1, ls="--")
stop_name = top_stops[node_idx]
axes[1].set_title(f"T-GCN | Stop {stop_name} | MAE={mae:.1f}")
axes[1].set_ylabel("Boardings / hour"); axes[1].legend(); axes[1].grid(alpha=0.3)

fig.suptitle("T-GCN Spatio-Temporal Forecast – SUNT Dataset", fontsize=13)
